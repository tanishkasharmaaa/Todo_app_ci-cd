...
export default function RegistrationPage() {
  ...
  const getRegistrationData = localStorage.getItem("RegistrationData")
  ...
  const onSubmit: SubmitHandler<RegistrationInputValues> = async (data) => {
    ...
    if (registrationData) {
      let parsedregistrationData = JSON.parse(registrationData);
      if (parsedregistrationData.email === data.email) {
        toast("Email already exists",{duration:4000});
        return;
      }
    }
      ...
      <div className="w-full max-w-md">
        <div className="rounded-2xl border border-blue-100 bg-white p-6 sm:p-8 shadow-xl">
          ...
          <p className="mt-6 text-center text-sm text-gray-500">
            ...
            <span className="cursor-pointer font-semibold text-blue-600 hover:text-blue-700" onClick={()=>window.location.replace("/login")}>
              Login
            </span>
          </p>
        </div>
